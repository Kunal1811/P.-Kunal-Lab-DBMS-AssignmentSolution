...
SELECT CUS_GENDER, COUNT(CUS_GENDER) AS TOTAL_COUNT, ORD_AMOUNT FROM Customer, `Order` 
WHERE Customer.CUS_ID = `Order`.CUS_ID AND ORD_AMOUNT >= 3000
GROUP BY CUS_GENDER;
...
/*INNER JOIN can be written as JOIN*/
SELECT `Order`.*,PRO_NAME FROM `Order` 
...
SELECT C.CAT_ID,C.CAT_NAME,P.PRO_NAME, MIN(SUPP_PRICE) AS Least_Expensive
...
/*Consumes a little bit more time because of cross join*/
SELECT Product.PRO_ID, Product.PRO_NAME 
FROM Product, Supplier_Pricing 
WHERE Product.PRO_ID = Supplier_Pricing.PRO_ID AND PRICING_ID IN (SELECT PRICING_ID FROM `Order` WHERE ORD_DATE > '2021-10-05');
...
DELIMITER &&
CREATE PROCEDURE Ssupplier_Ratings()
BEGIN
    ...
    WHERE Supplier_Pricing.SUPP_ID = Supplier.SUPP_ID AND Supplier_Pricing.PRICING_ID = `Order`.PRICING_ID AND `Order`.ORD_ID IN (SELECT ORD_ID FROM Rating)
    ORDER BY Supplier.SUPP_ID;
END &&

CALL Ssupplier_Ratings
